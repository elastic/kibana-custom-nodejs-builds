...
### Build on a branch
You can either create a new branch, where you set the default Node versions in [pipeline.yml](./.buildkite/pipeline.yml), and run a new build on that branch (or get the PR merged, and run a build on `main`). See [this PR](https://github.com/elastic/kibana-custom-nodejs-builds/pull/8) for reference.

### Default targets
By default, the pipeline uses a build matrix and builds custom artifacts for both:
- `22.17.1`
- `24.13.1`

### Build with override parameters
If you only want one target version, set an override parameter on buildkite when starting the job:
```
OVERRIDE_TARGET_VERSION=24.13.1
```
When this variable is set, the matrix is reduced to that single version.

The matrix pipeline is generated by [scripts/generate_buildkite_pipeline.sh](./scripts/generate_buildkite_pipeline.sh).

### Warning :warning:
If there are native modules going into production along with Kibana, each of them should be built custom along the builds we provide on this repo both for `glibc-217` and `pointer-compression` modes.

### What happens after?
Per target version, the job starts `(glibc-217 + pointer-compression)` builds for each platform (`amd64` + `arm64`) in parallel. The arm64 cross-compilation takes a while, but once each version is done, we create a `SHASUMS256.txt` based on the original version's hashes and update the entries for our custom-built artifacts.
